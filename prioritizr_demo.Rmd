...
fishing_cost <- raster(here("cleaned_data", "fishing_rast.tif"))

## Plot to see what it looks like
plot(fishing_cost)

## TEST WILL DELETE
mz_rast <- raster(here("cleaned_data", "mz_eez_rast.tif"))
area_cost <- raster(here("cleaned_data", "mz_eez_rast.tif"))

values(area_cost) <- 0.5
area_cost <- mask(area_cost, mz_rast)
...
## Baseline -30% target
prob_area_30 <- problem(fishing_cost, features = features_stack) %>%
  add_min_set_objective() %>%
