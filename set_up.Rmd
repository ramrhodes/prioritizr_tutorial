...
fishing_rast <- function(csv, mz_eez_sf){
  ...
  fishing_sf <-  st_as_sf(csv, coords = c("cell_ll_lon", "cell_ll_lat"), crs= 4326) %>% #make sf
    st_transform(., crs = st_crs(mz_eez_sf)) %>% #make sure it is in correct crs
    mutate(fishing_present = case_when(mmsi_present == NA~0, 
                                       TRUE ~ as.numeric(.$mmsi_present))) %>% #make all NAs 0s (for now)
    mutate(fishing_binary = case_when(fishing_present == 0 ~ 0.001, #Prioritzr doesn't like 0s
                                       fishing_present >= 1~1)) %>% #making binary
    mutate(id=1:n()) 
  ...
  crs(mz_eez_rast)
  ## make a raster 
  fishing_rast <- raster::rasterize(fishing_sp, mz_eez_rast, field = "fishing_binary") %>% 
    mask(mz_eez_sf)
